...
from toms_msg.srv import ArmService  # ★★★ RailServiceのインポートを削除 ★★★
...
class MasterControlNode(Node):
    def __init__(self):
        ...
        self.ee_command_publisher = self.create_publisher(String, 'end_effector_command', reliable_qos)
        self.joy_subscriber = self.create_subscription(Joy, 'joy', self.joy_callback, 10)
        self.arm_client = self.create_client(ArmService, "arm_service")
        # ★★★ レール関連のクライアント作成をコメントアウト ★★★
    ...
    def handle_manual_control(self):
        ...
        #     if self.latest_joy_msg.buttons[forward_btn] == 1 and not self.rail_button_pressed:
        #         self.rail_button_pressed = True
        #         playsound("/home/ylab/hibikino_toms_ws/src/cart_controller_pkg/cart_controller_pkg/sound/前.wav")
        #         self.send_rail_request('f')
        #     elif self.latest_joy_msg.buttons[backward_btn] == 1 and not self.rail_button_pressed:
        #         self.rail_button_pressed = True
        #         playsound("/home/ylab/hibikino_toms_ws/src/cart_controller_pkg/cart_controller_pkg/sound/後.wav")
        #         self.send_rail_request('b')
        #     elif self.latest_joy_msg.buttons[forward_btn] == 0 and self.latest_joy_msg.buttons[backward_btn] == 0:
        #         self.rail_button_pressed = False
        # else:
        #     self.rail_button_pressed = False
    ...
    def _check_async_task_done(self):
        if self.arm_future and self.arm_future.done():
            try:
                ...
                self.get_logger().error(f"アームサービス呼び出しエラー: {e}")
                self.trigger_reset_sequence()
                return
            finally:
                self.arm_future, self.sequence_step = None, self.sequence_step + 1
    ...
    def _reset_manual_state_from_yaml(self):
        try:
            ...
            arm_params_yaml = params['arm_params']
            self._reset_manual_state(arm_params_yaml)
        except (FileNotFoundError, KeyError) as e:
            self.get_logger().error(f"YAML読み込みエラー: {e}")
