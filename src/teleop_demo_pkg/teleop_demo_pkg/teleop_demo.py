...
class MasterControlNode(Node):
    def __init__(self):
        ...
        # (中略... 状態変数などの定義は前回と同じ)
        self.is_sequence_running = False; self.is_resetting = False; self.is_manual_locked = False; self.latest_joy_msg = None; self.rail_button_pressed = False
        self._reset_manual_state(arm_params_yaml)
        self.x_limit = {'min': -40.0, 'max': 250.0}; self.y_limit = {'min': 0.0, 'max': 500.0}; self.z_limit = {'min': 0.0, 'max': 450.0}; self.angle_limit = {'min': -90.0, 'max': 90.0}
        self.sequence_step = 0; self.wait_start_time = None; self.wait_duration = None; self.arm_future = None; self.arm_request = ArmService.Request()
        self.declare_parameter('joy_params.x_axis', 0); self.declare_parameter('joy_params.y_axis', 1); self.declare_parameter('joy_params.z_axis', 5); self.declare_parameter('joy_params.angle_axis', 2); self.declare_parameter('joy_params.suction_start_button', 2); self.declare_parameter('joy_params.suction_stop_button', 3); self.declare_parameter('joy_params.rail_forward_button', 5); self.declare_parameter('joy_params.rail_backward_button', 4)
        self.declare_parameter('scale.xy', 50.0); self.declare_parameter('scale.z', 50.0); self.declare_parameter('scale.angle', 90.0)
    ...
    def joy_callback(self, msg):
        ...
        if msg.buttons[start_btn] == 1 and not self.is_sequence_running and not self.is_resetting:
            ...
            self.is_sequence_running = True; self.sequence_step = 0
        elif msg.buttons[stop_btn] == 1 and not self.is_resetting:
            self.get_logger().info('======== システムをリセットします ========')
            self.is_sequence_running = False; self.arm_future = None; self.wait_start_time = None; self.is_resetting = True; self.sequence_step = 100
    ...
    # (中略... handle_manual_control と _check_async_task_done は変更なし)
    def handle_manual_control(self):
        if self.latest_joy_msg is None: return
        forward_btn = self.get_parameter('joy_params.rail_forward_button').value; backward_btn = self.get_parameter('joy_params.rail_backward_button').value
        if not self.is_manual_locked:
            if self.latest_joy_msg.buttons[forward_btn] == 1 and not self.rail_button_pressed: self.rail_button_pressed = True; self.send_rail_request('f')
            elif self.latest_joy_msg.buttons[backward_btn] == 1 and not self.rail_button_pressed: self.rail_button_pressed = True; self.send_rail_request('b')
        ...
        if not self.is_manual_locked:
            x_axis = self.get_parameter('joy_params.x_axis').value; y_axis = self.get_parameter('joy_params.y_axis').value; z_axis = self.get_parameter('joy_params.z_axis').value; angle_axis = self.get_parameter('joy_params.angle_axis').value; scale_xy = self.get_parameter('scale.xy').value; scale_z = self.get_parameter('scale.z').value; scale_angle = self.get_parameter('scale.angle').value
            self.current_pos.x -= self.latest_joy_msg.axes[x_axis] * scale_xy * 0.1; self.current_pos.y += self.latest_joy_msg.axes[y_axis] * scale_xy * 0.1; self.current_pos.z += self.latest_joy_msg.axes[z_axis] * scale_z * 0.1; self.current_angle_deg += self.latest_joy_msg.axes[angle_axis] * scale_angle * 0.1
            self.current_pos.x = max(self.x_limit['min'], min(self.x_limit['max'], self.current_pos.x)); self.current_pos.y = max(self.y_limit['min'], min(self.y_limit['max'], self.current_pos.y)); self.current_pos.z = max(self.z_limit['min'], min(self.z_limit['max'], self.current_pos.z)); self.current_angle_deg = max(self.angle_limit['min'], min(self.angle_limit['max'], self.current_angle_deg))
            pose_msg = Pose(); pose_msg.position = self.current_pos; angle_rad = math.radians(self.current_angle_deg); q = quaternion_from_euler(0, 0, angle_rad); pose_msg.orientation.x, pose_msg.orientation.y, pose_msg.orientation.z, pose_msg.orientation.w = q[0], q[1], q[2], q[3]
    ...
    def handle_harvest_sequence(self):
        ...
        elif self.sequence_step == 1: cmd = 'ENABLE_EDF'; self.get_logger().info(f'[STEP 1] 吸引を開始 ({cmd})'); self.ee_command_publisher.publish(String(data=cmd)); self.set_wait_timer(8.0)
        elif self.sequence_step == 2: cmd = 'CLOSE_FINGER'; self.get_logger().info(f'[STEP 2] 顎を閉じます ({cmd})'); self.ee_command_publisher.publish(String(data=cmd)); self.set_wait_timer(8.0)
        elif self.sequence_step == 3: self.get_logger().info('[STEP 3] 収穫ボックスへ移動'); self.arm_future = self.arm_send_request_box_async()
        elif self.sequence_step == 4: cmd = 'OPEN_FINGER'; self.get_logger().info(f'[STEP 4] 顎を開きます ({cmd})'); self.ee_command_publisher.publish(String(data=cmd)); self.set_wait_timer(8.0)
        elif self.sequence_step == 5: cmd = 'DISABLE_EDF'; self.get_logger().info(f'[STEP 5] 吸引を停止 ({cmd})'); self.ee_command_publisher.publish(String(data=cmd)); self.set_wait_timer(6.0)
        ...
        elif self.sequence_step == 7: self.get_logger().info('[STEP 7] アームとZ軸を初期位置へ移動します'); self.arm_future = self.arm_send_request_init_async()
    ...
    def handle_reset_sequence(self):
        ...
        elif self.sequence_step == 102: cmd = 'OPEN_FINGER'; self.get_logger().info(f'[RESET 3] 顎を開きます ({cmd})'); self.ee_command_publisher.publish(String(data=cmd)); self.set_wait_timer(5.0)
        ...
        elif self.sequence_step == 104: self.get_logger().info('[RESET 5] アームとZ軸を初期位置へ移動します'); self.arm_future = self.arm_send_request_init_async()
