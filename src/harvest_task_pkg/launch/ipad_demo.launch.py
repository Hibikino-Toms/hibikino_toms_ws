...
def generate_launch_description():
    ...
    vision_node = Node(
        package='vision_pkg',
        executable='cam_pub_service', # 'vision_service_node.py' ではない
        name='cam_pub_service',
        output='screen'
    )
    ...
    vision_streaming_node = Node(
        package='vision_pkg',
        executable='vision_service_ipad',
        name='vision_service_ipad'
    ) 

    #6. webからのリクエストを中継するノード
    ipad_demo_sync_node = Node(
        package='harvest_task_pkg',
        executable='ipad_demo_sync_relay'
    )
