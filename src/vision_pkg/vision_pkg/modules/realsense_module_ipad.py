...
import cv2
"""
RealSense for ipad モジュール
2025/11/06 映像ストリーミングと物体検出との競合を避けるための措置として、作成
...
class Realsense_Module():
    ...
    def get_cam_param(self):
        if self.color_intrinsics is None:
            raise RuntimeError("Realsense is not initialized. call setup() first.")
