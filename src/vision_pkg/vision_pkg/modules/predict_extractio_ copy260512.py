...
class Predict_Extra():
    ...
    def extraction_and_color_mask(self, results):
        ...
        # ★★★ 修正: ループの外側でマスク画像を初期化 ★★★
        # (self.IMG_HEIGHT, self.IMG_WIDTH) を使用
        color_mask = np.zeros((self.IMG_HEIGHT, self.IMG_WIDTH, 3), np.uint8)
        color_mask_main_stem = np.zeros((self.IMG_HEIGHT, self.IMG_WIDTH, 3), np.uint8)
        color_mask_peduncle = np.zeros((self.IMG_HEIGHT, self.IMG_WIDTH, 3), np.uint8)
        color_mask_tomato = np.zeros((self.IMG_HEIGHT, self.IMG_WIDTH, 3), np.uint8)
        # ★★★ 修正ここまで ★★★
