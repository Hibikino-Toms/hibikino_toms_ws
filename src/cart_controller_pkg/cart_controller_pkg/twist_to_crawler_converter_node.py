...
class TwistToCrawlerConverter(Node):
    def __init__(self):
        ...
        self.tread = 0.396  # 左右クローラー間の距離 [m]
        self.max_linear_speed = 0.217 # 変更後 (PWM 25.0 の時の実測値)

        self.get_logger().info(f'パラメータをコード内で設定: tread={self.tread}, max_linear_speed={self.max_linear_speed}')
