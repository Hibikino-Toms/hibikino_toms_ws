...
#         # Odom
#         #current_time = self.get_clock().now() - rclpy.duration.Duration(seconds=2.5)
#         odom = Odometry()
...
class CrawlerControllerSerial(Node):
    """
    修正版: Timerを使用して一定周期で送受信・オドメトリ発行を行うノード
    """
    ...
    def __init__(self):
        super().__init__('crawler_controller_serial') 
        self.get_logger().info('クローラー制御＆オドメトリノード (Timer版) を起動します。')
        
        self.serial_port = None
        # 自分の環境に合わせてパスを確認してください
        yaml_path = '/home/ylab/hibikino_toms_ws/module/set_params.yaml'
        
        try:
            params = self.load_yaml(yaml_path)
            crawler_params = params.get("crawler_params", {})
            self.serial_number = crawler_params.get('CRAWLER_PICO_SERIAL_NUMBER', None)
            self.baudrate = crawler_params.get('BAUDRATE', 115200)
            self.tread_width = crawler_params.get('TREAD', 0.4)
            
            if not self.serial_number:
                self.get_logger().warn("シリアルナンバーの設定が見つかりません。")
        ...
        # --- 重要: 制御ループ用タイマー (例: 20Hz = 0.05秒周期) ---
        # この中で「送信 -> 受信 -> オドメトリ計算 -> TF発行」を一括で行います
        if self.initialized_successfully:
            self.create_timer(0.05, self.control_cycle)
    ...
    def setup_serial(self):
        ...
        device_name = self.select_device(self.serial_number)
        if device_name is None:
            self.get_logger().error("指定されたシリアルナンバーのデバイスが見つかりません。")
            self.initialized_successfully = False
            return
        
        try:
            # タイムアウトは短めに設定し、制御ループをブロックしすぎないようにする
            self.serial_port = serial.Serial(device_name, self.baudrate, timeout=0.1)
            self.get_logger().info(f'{device_name} 接続成功')
    ...
    def pwm_callback(self, msg: Float32MultiArray):
        """PWM指令値を受け取って変数を更新するだけにする"""
        self.last_received_time = self.get_clock().now()
        self.current_pwm_left = msg.data[0]
        self.current_pwm_right = msg.data[1]

    def control_cycle(self):
        """
        一定周期(20Hz)で呼ばれるメインループ
        1. 安全停止チェック
        2. マイコンへ送信
        3. マイコンから受信＆オドメトリ更新
        """
        ...
        # 2. 送信 (Request)
        # バッファをクリアして、古いデータが混ざるのを防ぐ
        self.serial_port.reset_input_buffer()
        self.send_command_to_mcu(self.current_pwm_left, self.current_pwm_right)

        # 3. 受信待ち (Response)
        # Timer内であれば、多少の待機は許容されますが、無限ループは避けます
        try:
            line = self.serial_port.readline() # timeout=0.1 で設定しているので最大0.1秒待つ
            if line:
                pulse_data = line.strip().decode('utf-8', errors='ignore')
                self.calculate_and_publish_odom(pulse_data)
            else:
                # タイムアウト等でデータが来なかった場合でも、
                # TFの連続性を保つために「前回の位置」でTFを発行すべきですが、
                # ここではデータ欠損としてスキップします（必要に応じて前回値再送も検討）
                pass
        except Exception as e:
